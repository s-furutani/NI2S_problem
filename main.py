...
def simulate_results(graph, S, q, epsilon, X, kmax):
    xaxis = np.linspace(0, kmax, 21)
    ...
    for k in xaxis:
        X_k = X[:int(k)]
        q_X = update_q(q, epsilon, X_k)
        y_ave, _ = mtd.ICN_simulation(graph, S, q_X, num_simulation)
        Y_ave.append(y_ave)
    
    # end_time = time.time()
    # t = np.round(end_time - start_time, 2)
    # print(f"  execution time (simulation): {t} sec")
...
for i in range(9):
	...
	print('computing intervening nodes by FastMIOA')
	start_time = time.time()
	X = mtd.FastMIOA(new_graph, source, new_q, new_epsilon, kmax, theta)
	#check_property_of_interv_nodes(X, new_graph, new_q)  # agreement, degree, and distance of nodes in X can be checked by this function
	end_time = time.time()
	t = np.round(end_time - start_time, 2)
	print(f"  {t} sec")
	util.write_data(directory, 'FastMIOA', X)

	print('computing intervening nodes by Distance')
	start_time = time.time()
	X = mtd.BaselineDistance(new_graph, source, kmax)
	end_time = time.time()
	t = np.round(end_time - start_time, 2)
	print(f"  {t} sec")
	util.write_data(directory, 'Distance', X)

	print('computing intervening nodes by Degree')
	start_time = time.time()
	X = mtd.BaselineDegree(new_graph, source, kmax)
	end_time = time.time()
	t = np.round(end_time - start_time, 2)
	print(f"  {t} sec")
	util.write_data(directory, 'Degree', X)

	print('computing intervening nodes by Random')
	start_time = time.time()
	X = mtd.BaselineRandom(new_graph, source, kmax)
	end_time = time.time()
	t = np.round(end_time - start_time, 2)
	print(f"  {t} sec")
	...
	alg_names = ['GreedyMIOA', 'FastMIOA', 'Distance', 'Degree', 'Random', 'AdvancedGreedy']
	for alg_name in alg_names:
		...
		if os.path.exists(path) and not os.path.exists(path_sim):
			...
			results = simulate_results(graph, S, q, epsilon, X, kmax)
			filename = alg_name+'_sim_results'
			util.write_data(directory, filename, results)
